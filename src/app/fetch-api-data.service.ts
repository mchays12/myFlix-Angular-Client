...
const apiUrl = 'https://myflixappmatthew.herokuapp.com/';
 ...
 */
  ...
  // Making the api call for the getting one movie endpoint
  ...
  // Making the api call for the getting a director
  ...
  // Making the api call for the getting a genre
  ...
  // Making the api call for getting a user
  ...
  getFavoriteMovies(username: string, movieId: string): Observable<any> {
    const token = localStorage.getItem('token');
    return this.http.get(apiUrl + 'users/' + username + '/' + movieId, {
      headers: new HttpHeaders({
        Authorization: 'Bearer ' + token,
      })
    }).pipe(
  ...
  // Making the api call for editing user
